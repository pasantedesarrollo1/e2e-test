...
// Hosts that must never be targeted by the e2e suite.
const PRODUCTION_HOST_SUFFIXES = ["wanqara.app", "wanqara.net"];
// Non-production environments that live under a production suffix (allowed).
...
// Fails fast if PLAYWRIGHT_BASE_URL points at production, so the suite can never
// run against real customer data. Preview/staging and local hosts are allowed.
