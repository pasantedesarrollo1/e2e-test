...
export async function openAddWaybillDialog(page, { tenantBaseUrl }) {
  await ensureAuthenticated(page, { tenantBaseUrl, targetPath: "/admin/waybills/list" });
...
export async function fillAddressDetails(page, { address, reason, route, destinationSubsidiary }) {
  ...
  if (destinationSubsidiary) {
    ...
    const subsidiaryOption = page.locator(".v-list-item").filter({ hasText: destinationSubsidiary }).first();
    await subsidiaryOption.click();
    await expect(subsidiaryOption).not.toBeVisible();
  }
}
