...
import { addProductToCart, searchAndSelectProduct, selectTable, submitOrder } from "@/e2e/Wanqara/regression/POS/POS-R/harness/chef-orders-flow.js";

import { processOrderClosure } from "@/e2e/Wanqara/regression/POS/POS-R/harness/pos-close-order.js";

import { openExtrasSelection, addInStockExtra, confirmExtrasAndAddToCart } from "@/e2e/Wanqara/regression/POS/POS-R/harness/pos-extras-helpers.js";

export async function navigateToRestaurantPOS(page: Page, subsidiaryName: string): Promise<void> {
  ...
  if (await loginBtn.isVisible()) {
    const loginCredentials = (playwrightHarness.users as Record<string, ChefUserCredentials | undefined>)["restaurant"];
    if (!loginCredentials) throw new Error("playwrightHarness.users.restaurant is not defined");
...
export type ExtraItem = string | { name: string; [key: string]: unknown };

export interface ChefOrderExtras {
  categoryName: string;
  items: ExtraItem[];
}

export interface CreateChefOrderOptions {
  productName: string;
  quantity?: number;
  chefLogin?: ChefUserCredentials;
  chefSubsidiary?: string;
  chefSubsidiaryCode?: string;
  extras?: ChefOrderExtras | null;
}

export async function createChefOrder(page: Page, {
  productName,
  quantity = 1,
  chefLogin,
  chefSubsidiary,
  chefSubsidiaryCode,
  extras = null
}: CreateChefOrderOptions): Promise<string> {
  ...
  const tableName: string = await selectTable(page);
  ...
  if (extras) {
    ...
    for (const extra of extras.items) {
      ...
      await addInStockExtra(page, extraName);
    }
...
export async function finalizeSaleWithPayment(page: Page, cedula: string, paymentMethod: string): Promise<void> {
...
export async function addProductToExistingOrder(page: Page, productName: string): Promise<void> {
...
export async function collectOrder(page: Page): Promise<void> {
...
export async function navigateToCloseOrderFromOptions(page: Page): Promise<void> {
...
export async function navigateToChangeOrderStatusFromOptions(page: Page): Promise<void> {
  const triggerLocator = page.getByRole("button", { name: /Más Opciones/i }).first();
...
export async function closeAllActiveOrders(page: Page, subsidiaryName: string, reason: string): Promise<void> {
