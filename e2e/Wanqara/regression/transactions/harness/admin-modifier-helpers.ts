...
export async function applyGeneralDiscount(page: Page, rate: string | number): Promise<void> {
...
export async function applyManualSurcharge(page: Page, rate: string | number): Promise<void> {
  ...
  await Promise.all([
    page.waitForResponse(
      (res: any) => res.url().includes("/api/v1/pos") && res.request().method() === "POST",
      { timeout: 10000 }
    ).catch(() => {}),
