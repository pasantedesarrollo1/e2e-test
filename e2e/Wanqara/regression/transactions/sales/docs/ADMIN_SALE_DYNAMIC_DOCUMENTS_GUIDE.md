# Admin Sale Dynamic Documents Architecture Guide

## Overview
This directory implements the **Modular Data-Driven Testing (DDT)** architecture applied across the repository for Transactions / Sales / Admin Sale Dynamic Documents.

**For AI Agents:** This test is specifically tailored to verify that the **Document Type** dropdown dynamically updates its available options based on the **Subsidiary** (Sucursal) currently selected in the UI. Subsidiaries with electronic invoicing enabled should show `Factura electrónica`, while those without it should fallback to `Recibos`.
...
*   `waitForFormDefaults`: Ensures that the form fields (Subsidiary, Warehouse, Document Type) are completely loaded and not in a disabled/loading state.
...
*   `admin-sale-dynamic-documents.spec.js`: Feeds the JSON contract into the orchestrator logic. It drives the browser to execute a sequence of context switches defined in the `steps` array.

---

## 2. The JSON Contract & Co-Location

### Example `0-json-data/admin-sale-dynamic-documents.json`
*   **`targetPath`**: The URL to start the test (e.g., `"/admin/ventas/add"`).
*   **`steps`**: An array of sequential interactions.
