...
import { createSize, deleteSize } from "./harness/size-helpers.js";

import scenarios from "./0-json-data/sizes-delete.json" assert { type: "json" };

test.describe("Inventory - Sizes (Delete)", () => {
  ...
  for (const scenario of scenarios) {
    ...
    describeBlock(`Escenario: ${scenario.description} ${executionTag}`, () => {
      ...
      test(`ensures size '${scenario.sizeData.name}' can be deleted`, async ({ page }) => {
        ...
        await test.step('Acción: Eliminar la talla', async () => {
