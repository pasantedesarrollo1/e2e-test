...
import { createSize, searchSize } from "./harness/size-helpers.js";

import scenarios from "./0-json-data/sizes-search.json" assert { type: "json" };

test.describe("Inventory - Sizes (Search)", () => {
  ...
  for (const scenario of scenarios) {
    ...
    describeBlock(`Escenario: ${scenario.description} ${executionTag}`, () => {
      ...
      test(`ensures size '${scenario.sizeData.name}' can be found via search`, async ({ page }) => {
        await test.step('Precondición: Crear la talla por UI', async () => {
          await createSize(page, {
            ...scenario.sizeData,
            tenantBaseUrl
          });
        });
        await test.step('Acción: Buscar la talla', async () => {
