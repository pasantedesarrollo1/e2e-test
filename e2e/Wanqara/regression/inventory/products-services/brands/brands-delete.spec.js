...
import { deleteBrand, createBrand } from "./harness/brand-helpers.js";

import scenarios from "./0-json-data/brands-delete.json" assert { type: "json" };

test.describe("Inventory - Brands (Delete)", () => {
  ...
  for (const scenario of scenarios) {
    ...
    describeBlock(`Escenario: ${scenario.description} ${executionTag}`, () => {
      ...
      test(`ensures brand '${scenario.brandData.name}' can be deleted`, async ({ page }) => {
        ...
        await test.step('Acción: Eliminar la marca', async () => {
