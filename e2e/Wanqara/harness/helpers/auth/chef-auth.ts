...
export const getChefSessionPath = (chefAuthType: string = "actor1"): string => path.resolve(
  path.dirname(fileURLToPath(import.meta.url)),
  `../../.auth/chef-${chefAuthType}-session.json`
);

export interface LoginChefOptions {
  chefBaseUrl?: string;
  login: ChefUserCredentials;
  subsidiary: string;
  subsidiaryCode: string;
}

export async function loginChef(page: Page, { chefBaseUrl, login, subsidiary, subsidiaryCode }: LoginChefOptions): Promise<void> {
...
export interface EnsureChefAuthOptions {
  chefBaseUrl?: string;
  targetPath: string;
  login?: ChefUserCredentials;
  subsidiary?: string;
...
export async function ensureChefAuthenticated(page: Page, { chefBaseUrl, targetPath, login, subsidiary, subsidiaryCode, chefAuthType = "actor1" }: EnsureChefAuthOptions): Promise<void> {
  const effectiveLogin = login || chefHarness.users[chefAuthType as "actor1"];
  ...
  const defaultLogin = chefHarness.users[chefAuthType as "actor1"];
