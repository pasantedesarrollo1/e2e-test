...
const env = { name: 'Restaurant', authType: 'restaurant', fixture: 'posRestaurantPage', paymentUrl: /\/pos\/restaurant-payments/ };
...
test.describe(`POS Sales Rounding Error Cases - ${env.name} @release`, () => {
  ...
  for (const tc of roundingCases) {
    ...
    test(testTitle, async ({ posRestaurantPage: page }) => {
      ...
      if (tc.expectedPaymentError) {
        ...
        await Promise.all([
          page.waitForResponse(res => res.url().includes('/api/v2/pos/sales/restaurant') && res.request().method() === 'POST' && res.status() === 200),
