...
export async function selectDeliveryMode(page, modal) {
  const deliveryOption = modal.locator("div").filter({ hasText: /^DeliveryEnvío a domicilio$/ }).nth(1);
...
export async function ensureDeliveryPhoneAndAddress(page, modal, phone) {
  ...
  const addressCard = page.locator(".tw-aspect-video.tw-w-64").first();
  const warning = page.locator(".tw-border-warning").filter({ hasText: /No se encontraron direcciones/i }).first();

  await expect(addressCard.or(warning)).toBeVisible();
...
export async function addClientFromDeliveryForm(page, form, { cedula }) {
  const addClientBtn = form.locator(".v-btn.v-btn--flat.v-btn--icon.bg-primary.tw-mt-1").first();
  await addClientBtn.click();

  const clientModal = page.locator(".v-overlay__content").filter({ has: page.locator("#identity-input") }).first();
  ...
  const searchBtn = clientModal.locator(".v-btn.bg-primary.tw-p-0").first();
...
export async function saveDeliveryForm(page) {
  const saveBtn = page.getByRole("button", { name: /^Guardar$/i }).last();
...
export async function selectExistingDeliveryAddress(page) {
  ...
  await expect(page.locator(".v-chip").filter({ hasText: /Seleccionada/i }).first()).toBeVisible();
}
